...
<div class="backbj clearfix">
   ...
   <div class="q_l_btbig a666_b">图片浏览</div>
   
   
        <div class="case clearfix">
		<div class="q_pic_syousai">
			
		    <div class="q_pic_next">({$no}/{$photonum})<a name="here" href="grp_photo.php?module=nextPage&ID={$rs.main.ID}&photoID={$photoMsg.base.ID}&key={$no}">下一张</a>&nbsp;&nbsp;<a href="grp_photo.php?ID={$rs.main.ID}?">返回列表</a></div>
			<div class="q_pic_pic"><img src="./uploadfiles/group/{$rs.main.ID}/group_photo/{$photoMsg.base.realname}" /><p>{$photoMsg.base.oldname}</p></div>
		   ...
		   <td valign="top" class="nr">
				...
				<a href="#" onclick="photoReplyDel('photo','{$item.ID}','{$rs.main.ID}','{$photoMsg.base.ID}','{$no}');">删除</a>
				{/if}
				</td>
			  </tr>
			  <!--若点击回复，请用下面的代码-->
		  ...
		  <!--发表评论-->
		   ...
		   <form action="grp_photo.php?module=review&photoID={$photoMsg.base.ID}&ID={$rs.main.ID}" method="post" id="frm2" name="frm2">

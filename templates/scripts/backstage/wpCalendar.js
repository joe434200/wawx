...
{   
    ...
    else{WdatePicker(obj,strFormat,x,y)}   
}   
  
//选择日期 → 由 寒羽枫 2006-06-25 添加   
function WdatePicker(obj,strFormat,x,y)   
{   
  
	//------------------add by lzg------------------------------------
	if (obj == null || obj=='undefined')obj=event.srcElement;
	
	if (strFormat == null || strFormat == 'undefined')strFormat="yyyy-MM-dd";//
	//---------------add by lzg-----------------------------------
    ...
    var date = new Date();   
    var by = date.getFullYear()-10;  //最小值本年   
    var ey = date.getFullYear()+90;  //最大值 → 50 年后   
...
Calendar.prototype.draw = function() {   
  ...
  mvAry[mvAry.length]  = '        <th align="left" width="1%"><input class="NavImgll"  style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;';   
  if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏“月”   
  mvAry[mvAry.length]  ='" name="prevYear" type="button" id="prevYear"  /></th>'; 
  mvAry[mvAry.length]  = '        <th align="left" width="1%"><input class="NavImgl"  style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;';   
  if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏“月”   
  mvAry[mvAry.length]  ='" name="prevMonth" type="button" id="prevMonth"  /></th>';   
  ...
  mvAry[mvAry.length]  = '        <th align="right" width="1%"><input class="NavImgr" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;';   
  if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏“月”   
  mvAry[mvAry.length]  ='" name="nextMonth" type="button" id="nextMonth"  /></th>'; 
  mvAry[mvAry.length]  = '        <th align="right" width="1%"><input class="NavImgrr" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;';   
  if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏“月”   
  mvAry[mvAry.length]  ='" name="nextYear" type="button" id="nextYear"  /></th>'; 
  mvAry[mvAry.length]  = '      </tr>';   
...
Calendar.prototype.goPrevYear = function(e){   
  ...
  if (this.year == this.beginYear ){alert('年未満の最小年設置できない'); return;}  
...
Calendar.prototype.goNextYear = function(e){ 
  ...
  if (this.year == this.endYear ){alert('年より最大年設置のない');return;} 
...
{   
    var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:9999; background-color: #FFFFFF;border: 1px solid #CCCCCC;width:180px;font-size:12px;"></div>';   
    if(document.all)   
    {   

	        ...
	        objXmlHttp.onload = function () 
	        {
    				    ...
    				    switch(CurrentDivID)
                        {
                            ...
                            case "DivUserPanel":
                                ...
                                break;
                            case "hdFlag1": case "hdFlag2": case "hdFlag3":
                                HD(CurrentDivID,'1');
                                break;
...
function OperatePaiHangDiv(divID)
{
	...
	var obj=null;
    var objShowPanel=null;
    objShowPanel=_gObjByID("hd_ShowPanel");
    var shopPanel_="hd_ShowPanel";
	if(CurrentShopPaiHangFlag!=divID)
	{	        	        
	        for(var i=1;i<=3;i++)
	        {
		        _gObjByID("hd_Flag" + i).removeAttribute("className");
		        _gObjByID("hd_Flag" + i).removeAttribute("class");
	        }
	        _gObjByID("hd_Flag" + divID).className='current';
        	
	        
	        if(CurrentShopPaiHangFlag!="")
	        {
				//alert('CurrentCircleFlag!=""');
	            obj=_gObjByID("hd_Flag" + divID + "_Content");
		        _gObjByID("hd_Flag" + CurrentShopPaiHangFlag + "_Content").innerHTML=objShowPanel.innerHTML;
				objShowPanel.innerHTML=GetLoadingImg();
		        if(obj.innerHTML == null || obj.innerHTML == "" || obj.innerHTML == GetLoadingImg())
	            {//alert('GetFromAspx');
//		            var oXmlHttp = createXMLHttp();		
//	                ShowDivContent('indexContent/hd.aspx?Flag=' + divID,oXmlHttp,"hdFlag"+divID,objShowPanel);
	                
	                var x = new XHR("re_OperatePaiHangDiv('hd_ShowPanel','hdFlag"+divID+"')");
 	                //alert(url);
 	                var url = "indexContent/hd.aspx?Flag=" + divID ;
	                x.get(url);
	            }
	            else
	            {//alert('GetFromDiv');
...
//	                ShowDivContent('indexContent/DivShopWantOrGo.aspx?Flag=' + divID,oXmlHttp,"DivShop"+divID,objShowPanel);
	        ...
	        {
//	                var oXmlHttp = createXMLHttp();		
//	                ShowDivContent('indexContent/hd.aspx?Flag=' + divID,oXmlHttp,"DivShop"+divID,objShowPanel);
	                var x = new XHR("re_OperateGoAndWantDiv('DivShopWantOrGo_ShowPanel','DivShop"+divID+"')");
 	                //alert(url);
 	                var url = "indexContent/DivShopWantOrGo.aspx?Flag=" + divID ;
	                x.get(url);

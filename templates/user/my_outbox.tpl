...
<div class="my_right clearfix">
  ...
  <tr>
    ...
    <td width="90%">(共 {$info.base.counts} 封)</td>
   </tr>
   </table>
   </div>
  ...
  <tr>
    ...
    <td width="12%" align="center"><input name="Input" type="button"  value="删除"  onclick="javascript:AjaxDelBox(1,'{$item.ID}')"/></td>
  </tr>
  <tr valign="top" class="pic">
  <td>&nbsp;</td>
  </tr>
  ...
  {/if}
   ...
   {if $info.info|@count neq 0}
		...
		{else}
		    ...
		    {else}
		    <a  href="javascript:void(0)" onclick="javascript:AjaxGetBox('1','{$info.base.page-1}')">上一页</a>
		    {/if}
		    {section name=loop loop=$info.base.pageCounts}
		    	...
		    	{else}
		    	&nbsp;<a href="javascript:void(0)" onclick="javascript:AjaxGetBox('1','{$smarty.section.loop.index+1}')">{$smarty.section.loop.index+1}</a>
		    	{/if}
		    {/section}
			{if $info.base.page eq $reply.base.pageCounts}
			&nbsp;<a class="pageFooterStyle">下一页</a>
			{else}
			&nbsp;<a  href="javascript:void(0)" onclick="javascript:AjaxGetBox('1','{$info.base.page+1}')">下一页</a>
			{/if}
		{/if}
	{/if}
